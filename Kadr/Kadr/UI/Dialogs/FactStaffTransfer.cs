...
namespace Kadr.UI.Dialogs
{
    public partial class FactStaffTransfer : CustomBaseDialog
    {
        ...
        private void planStaffLoad()
        {
            ...
            IEnumerable<PlanStaff> PlanStaff = KadrController.Instance.Model.PlanStaffs.Where(plStaff => (plStaff.Dep.id == currentDepartment.id)
                                            && (plStaff != currentPlanStaff) && (plStaff.Prikaz == null)).OrderByDescending(plStaff => plStaff.Prikaz.DatePrikaz).ToArray();

            ArrayList freePlanStaff = new ArrayList();

            foreach (PlanStaff planStaff in PlanStaff)
            {
                if (planStaff.FreeStaffCount > 0)
                    freePlanStaff.Add(planStaff);
            }
            planStaffBindingSource.DataSource = freePlanStaff;
        }
