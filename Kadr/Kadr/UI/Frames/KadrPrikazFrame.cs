...
namespace Kadr.UI.Frames
{
    public partial class KadrPrikazFrame : KadrBaseFrame
    {
        ...
        {
            prikazDecoratorBindingSource.DataSource = KadrController.Instance.Model.Prikazs.OrderByDescending(prik => 
                prik.DatePrikaz).ThenByDescending(prik => prik.PrikazName);
        }
        ...
        private void btnPrikazToExcel_Click(object sender, EventArgs e)
        {
            Kadr.Controllers.ExcelExportController.Instance.ExportToExcel(dgvPrikaz);
        }
