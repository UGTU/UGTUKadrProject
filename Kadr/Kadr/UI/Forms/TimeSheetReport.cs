...
namespace Kadr.UI.Forms
{
    ...
    public partial class TimeSheetReport : Form
    {
        ...
        private void cbTimeSheet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            IEnumerable<GetFactStaffForTimeSheetResult> NotCreatedFactStaff = CurrentTimeSheet.GetNotInsertedStaff(staff).Where(st => st.idTypeWork != 15).ToArray();
                ...
                ((from st in NotCreatedFactStaff
                 ...
                 select new
                 {
                     ...
                     FactSt = fcSt
                        ...
                        (!tsRecord.IsClosed)).Select(tsRecord => new TimeSheetRecord(tsRecord)))).OrderBy(tsRecord => 
        ...
        {
            foreach (var tShRecord in CurrentTimeSheet.TimeSheetFSWorkingDays.Where(tsRecord =>
                        (!tsRecord.IsClosed)))
            {
                tShRecord.IsClosed = true;
            }
            KadrController.Instance.Model.SubmitChanges();
        }
    }
