...
namespace Kadr.KadrTreeView
{
    ...
    public class RootNodeObject : KadrNodeObject
    {
        ...
        protected override bool DoAddChildNodes()
        {
            ...
            IOrderedQueryable<FactStaff> FactStaff = KadrController.Instance.Model.FactStaffs.Where(factSt =>
                    (factSt.PlanStaff.Dep == department) || (factSt.Dep == department)).OrderBy(factSt => factSt.Employee.EmployeeName);
            //создаем узлы-сотрудники в соответствии с фильтром
                //проверяем на соответствие фильтру
            Employee empl = null;    
            foreach (FactStaff factSt in FactStaff)
                {
