...
--SELECT * from [dbo].[GetDepartmentBonusWithSettings](1, GETDATE(),GETDATE(),1,1,-1) WHERE BonusLevel='Штатное расписание'


--SELECT * from [dbo].[GetDepartmentBonusWithSettings](1, '01.1.2015','19.1.2015',1,12,-1) order by EmployeeName where /*idFactStaff is not null and MatOtpusk is null and*/ idEmployee=201342   BonusTypeName is null
--SELECT * from [dbo].[GetDepartmentBonusWithSettings](1, '01.1.2015','19.1.2015',1,3,2) order by EmployeeName where /*idFactStaff is not null and MatOtpusk is null and*/ idEmployee=201342   BonusTypeName is null
...
--SELECT * from [dbo].[GetDepartmentBonusWithSettings](623, '01.1.2015','31.01.2015',1,8,-1) where idEmployee=826866 TypeWorkName is not null
--SELECT * from [dbo].[GetDepartmentBonusWithSettings](16, '01.8.2015','31.08.2015',1, 12, -1)
...
CREATE FUNCTION [dbo].[GetDepartmentBonusWithSettings] 
(
@idDepartment INT,
...
RETURNS @Result TABLE
   (
    ReportMainObjectName	VARCHAR(500),
    TypeWorkName			VARCHAR(50) NULL,
    PKCategoryName			VARCHAR(250),
	...
	DepartmentFullName		VARCHAR(500),
	...
	PrikazAppointment   VARCHAR(50),
	NewCategoryName			VARCHAR(50),
	[FundingCenterName] VARCHAR(500)
   ) 
AS
BEGIN
...
--объявляем временную таблицу, в которую внесем все надбавки за период
	...
	BEGIN
		...
		INSERT INTO @BonusTable(id, BonusCount, idBonusType, BonusFinancingSourceName, 
				Comment,DateBegin,DateEnd,IntermediateDateEnd,IntermediatePrikaz, PrikazAppointment)
		SELECT idBonus, PeriodBonus.BonusCount, Bonus.idBonusType, BonusFinancingSource.FinancingSourceName,
				Comment,PeriodBonus.DateBegin,PeriodBonus.DateEnd,Prikaz.DatePrikaz,Prikaz.PrikazName, PrikazAppointment
...
--объявляем временную таблицу, в которую внесем все ОТДЕЛЫ, по которым нужно вывести данные
	...
	SELECT idDepartment, idManagerPlanStaff, DirectionManagerName, DepartmentName,DepartmentSmallName, DepTreeIndex, [idFundingCenter]
...
--объявляем временную таблицу, в которую внесем всех сотрудников отдела в периоде
	...
	INSERT INTO @StaffTable(idDepartment, idFinancingSource, idTypeWork, idPost, idPlanStaff, idFactStaff, idEmployee, StaffCount, 
		...
		DepartmentName,DepartmentSmallName, DepTreeIndex, [OKVEDName], [FundingCenterName])
	SELECT PeriodStaff.idDepartment, idFinancingSource, PeriodStaff.idTypeWork, idPost, PeriodStaff.idPlanStaff, 
		...
		DepartmentName,DepartmentSmallName, DepTreeIndex, [OKVEDName], FundingCenter.FundingCenterName
	FROM dbo.GetStaffByPeriod(@PeriodBegin,@PeriodEnd) as PeriodStaff 
		...
		LEFT JOIN [dbo].[OKVED] ON PeriodStaff.idOKVED=[OKVED].id
		...
		LEFT JOIN dbo.FundingCenter ON FundingCenter.id=ISNULL(FactStaff.idFundingCenter,Deps.idFundingCenter)
	...
	INSERT INTO @Result(ReportMainObjectName, EmployeeName, BonusLevel, BonusCount, idFactStaff, SeverKoeff, RayonKoeff,
			...
			Comment,IntermediateDateEnd,IntermediatePrikaz, DepTreeIndex, OnlyExtrabudgetary, idWorkType, PrikazBegin, idDepartment,
			[CategoryVPOName],[CategoryVPOOrderBy],[CategoryZPName],[CategoryZPOrderBy], [OKVEDName], PrikazAppointment, NewCategoryName,[FundingCenterName])
	SELECT PlanStaff.DepartmentName, AllBonus.EmployeeName, AllBonus.BonusLevel, AllBonus.BonusCount, 
			...
			GlobalPrikaz.PrikazNumber GlobalPrikazFullName,
			BonusSuperType.BonusSuperTypeName, Category.CategorySmallName, Post.idCategory,
			...
			idBonusMeasure,IsStaffRateable,IsCalcStaffRateable,
			AllBonus.Comment,IntermediateDateEnd,IntermediatePrikaz, DepTreeIndex, OnlyExtrabudgetary, WorkType.id, PrikazBegin, PlanStaff.idDepartment,
			[CategoryVPOName],CategoryVPO.OrderBy,[CategoryZPName],CategoryZP.OrderBy, [OKVEDName], PrikazAppointment, NewCategory.CategorySmallName, [FundingCenterName]
	FROM
	...
	SELECT DISTINCT Employee.EmployeeName,
		...
		NULL ForVacancy,NULL ForEmployee, BonusFinancingSourceName, 
				Comment,ISNULL(Bonus.DateBegin, Staff.DateBegin) DateBegin, ISNULL(Bonus.DateEnd, Staff.DateEnd) DateEnd,IntermediateDateEnd,IntermediatePrikaz, PrikazBegin, PrikazAppointment
	...
	SELECT ISNULL(Employee.EmployeeName, 'Вакансия'),
		...
		NULL ForVacancy,NULL ForEmployee, NULL, 
				null,DateBegin, DateEnd,null,null,null, NULL
	FROM @StaffTable as Staff 
		...
		LEFT JOIN [dbo].[CategoryVPO] ON Post.idCategoryVPO=[CategoryVPO].id
		LEFT JOIN [dbo].[CategoryZP] ON Post.idCategoryZP=[CategoryZP].id
		LEFT JOIN [dbo].[Category] NewCategory ON Post.idNewCategory=NewCategory.id
...
go
SELECT DISTINCT
DepartmentFullName, PostName, BonusTypeFullName, BonusCount, FinancingSourceFullName, BonusFinancingSourceName, PrikazBegin
 from  [dbo].[GetDepartmentBonusWithSettings](1, '05.1.2014','05.10.2014',1,1,-1)bons
 inner join [dbo].[GetBonusByPeriod]('05.1.2014','05.10.2014')bonus
 ON bons.
