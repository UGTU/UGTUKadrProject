...
--select * from [dbo].[GetBonusByBonusType](67, '09.01.2013','09.30.2013') where EmployeeName like '%Амосова%'
--возвращает данные для отчета по надбавкам, отобранным по виду надбавки
CREATE FUNCTION [dbo].[GetBonusByBonusType_Prev] 
(
@idBonusType INT,
@PeriodBegin	DATE,
@PeriodEnd	DATE
...
--объявляем временную таблицу, в которую внесем всех сотрудников в периоде
	...
	*/
	...
	INSERT INTO @Result(EmployeeName,BonusLevel,BonusCount, BonusSum, idFactStaff, SeverKoeff, RayonKoeff,
			...
			ForEmployee,MatOtpusk, SalaryKoeff, FinancingSourceName, RealFinancingSourceName, CalcStaffCount)
	SELECT  EmployeeName, 
			...
			FinancingSource.FinancingSourceName, RealFinancingSource.FinancingSourceName, CalcStaffCount
	FROM
		(SELECT idBonus,
