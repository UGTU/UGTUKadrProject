...
SELECT     dbo.PlanStaff.id, dbo.PlanStaffHistory.StaffCount, dbo.PlanStaff.idDepartment, dbo.PlanStaff.idPost, dbo.PlanStaffHistory.idBeginPrikaz, 
                      ...
                      dbo.PlanStaffHistory.DateBegin AS DateBeginR
FROM         dbo.PlanStaff INNER JOIN
                      dbo.PlanStaffHistory ON dbo.PlanStaff.id = dbo.PlanStaffHistory.idPlanStaff 
					  INNER JOIN
					  (SELECT   PlanStaffHistory.idPlanStaff,  MAX(id) AS Lastid
                            FROM          dbo.PlanStaffHistory
                            WHERE      (DateBegin <= GETDATE())
							GROUP BY PlanStaffHistory.idPlanStaff)LastPlanStaffHistory
							ON (LastPlanStaffHistory.idPlanStaff = dbo.PlanStaff.id) 
								AND  dbo.PlanStaffHistory.id =LastPlanStaffHistory.Lastid
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'PlanStaffCurrent';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'PlanStaffCurrent';
